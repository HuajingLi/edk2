  ...
  )
{
  ...
  if (CurDir != NULL) {
    if (EFI_ERROR(ShellFileExists (CurDir))) {
      //
      //EfiShellSetCurDir function cannot set "cwd" variable to NULL.
      //EfiShellSetEnv funtion is not allowed setting the "cwd" variable.
      //Only InternalEfiShellSetEnv function is allowed setting the "cwd" variable.
      //
      InternalEfiShellSetEnv (L"cwd", NULL, TRUE);
