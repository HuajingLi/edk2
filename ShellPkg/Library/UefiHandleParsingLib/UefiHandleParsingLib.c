...
{
  return GetProtocolDumpInformation(TheHandle, Verbose, &gEfiDevicePathProtocolGuid);
}
...
{
  return GetProtocolDumpInformation(TheHandle, Verbose, &gEfiLoadedImageDevicePathProtocolGuid);
}
  ...
  )
{
  ...
  //
  Buffer = AllocatePool (BlockMedia->BlockSize);
  if (Buffer) {
    BlockIo->ReadBlocks (
              BlockIo,
              BlockMedia->MediaId,
